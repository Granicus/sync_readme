...
describe SyncReadme do
  ...
  describe '#invoke' do
    context 'has default in config' do
      before :each do
        SyncReadme::Config::DEFAULT_CONFIG_FILE = 'spec/fixtures/config_yml/multiple_entries.yml'.freeze
      end
    ...
    context 'no default in config' do
      before :each do
        SyncReadme::Config::DEFAULT_CONFIG_FILE = 'spec/fixtures/config_yml/no_default_multiple.yml'.freeze
      end
      it 'runs on all when no default provided' do
        expect(SyncReadme).to receive(:perform).twice.with(instance_of(String))
        SyncReadme.invoke([])
      end
    end
